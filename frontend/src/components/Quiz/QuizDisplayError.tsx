...
export default function QuizDisplayError({ errorMessage, quizName ,setError,req, language}: QuizErrorProps) {
  const lang = (language as QuizComponentsLanguage) || 'English';
    ...
    <GlobalContainer fullScreen={false}>
      <QuizHeader>
        <QuizTitle>
          ...
          {quizName}{` - ${quizTexts.generationFailedSuffix.replace('{quizName} - ', '')}`}
        </QuizTitle>
      </QuizHeader>
      ...
      <Container>
        <ErrorState>
          <ErrorIcon>
           ...
           <ErrorTitle>{quizTexts.quizGenerationFailed}</ErrorTitle>
           
           <p>
             {quizTexts.quizNotValid}
           </p>
