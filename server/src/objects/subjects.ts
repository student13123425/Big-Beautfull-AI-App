...
export class FishierMaterie {
  ...
  // Tracks async text extraction state to prevent race conditions
  ...
  constructor(path: string, materie: string, save: Function, isFirst: boolean, config: Config) {
    ...
    // FIX #2: Wrap extraction in a tracked promise so we can await it later
    if (isFirst) {
      ...
      this._isContentLoaded = true; // Assume already loaded if not first time
    }
  }
  ...
  // Helper to safely wait for content extraction if it's still running
  ...
  async genereaza_sinteza(models: ModelInfo[], url: string | null, onUpdate: Function, config: Config, setError: (error: AiServerError) => void): Promise<void> {
    ...
    // FIX #4: Prevent duplicate/overlapping calls
    ...
    // FIX #2: Wait for content extraction to finish before proceeding
    ...
    // 🔧 RESTORED: Use url exactly as passed (device_ip) with explicit validation
    ...
    try {
      ...
      const nume_model: string = config.ai_model_sinteza?.[0]?.toLowerCase() || "";
      ...
      this.sinteza = await get_compleation(
        ...
        () => this.is_computing,
        url, // ✅ Uses device_ip/url exactly as passed before
        model_full.path,
        null,
  ...
  async generateHTML(models: ModelInfo[], url: string | null, onUpdate: Function, config: Config, setError: (error: AiServerError) => void, style: number): Promise<void> {
    ...
    try {
      ...
      // ✅ Fixed callback to update html_file instead of sinteza (original had a copy-paste bug)
